...
pool_alloc *new_pool_alloc(int chunk_size, int block_size, int initial_size) {
...
void delete_pool_alloc(pool_alloc *alloc) {
  // Free blocks
...
block *new_block(int chunk_size, int block_size) {
  ...
  void *chunks = calloc(block_size, (sizeof(chunk) + chunk_size));
  if (chunks == NULL) {
    free(_block);
    return NULL;
  }
  ...
  for (int index = 0; index < block_size - 1; ++index) {
    current->next = (chunk *)((char *)current + (sizeof(chunk) + chunk_size));
...
void *malloc_chunk(pool_alloc *alloc) {
  ...
  void *payload = (char *)alloc->alloc_ptr + sizeof(chunk);
...
void free_chunk(pool_alloc *alloc, void *payload) {
  chunk *payload_chunk = (chunk *)((char *)payload - sizeof(chunk));
