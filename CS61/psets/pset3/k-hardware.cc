...
void check_pagetable(x86_64_pagetable* pagetable) {
    ...
    assert(vmiter(pagetable, (uintptr_t)exception_entry).pa() == kptr2pa(exception_entry));
    assert(vmiter(kernel_pagetable, (uintptr_t)pagetable).pa() == kptr2pa(pagetable));
    assert(vmiter(pagetable, (uintptr_t)kernel_pagetable).pa() == kptr2pa(kernel_pagetable));
}
...
bool allocatable_physical_address(uintptr_t pa) {
    ...
    return !reserved_physical_address(pa) && (pa < KERNEL_START_ADDR || pa >= round_up((uintptr_t)_kernel_end, PAGESIZE)) && (pa < KERNEL_STACK_TOP - PAGESIZE || pa >= KERNEL_STACK_TOP) && pa < MEMSIZE_PHYSICAL;
}
...
int pcistate::next(int addr) const {
    ...
    while (true) {
        if (addr_func(addr) == 0 && (x == uint32_t(-1) || !(x & 0x800000))) {
...
__no_asan bool lookup_symbol(uintptr_t addr, const char** name, uintptr_t* start) {
    ...
    while (l < r) {
        ...
        if (sym.st_value <= addr && (m + 1 == symtab.nsym ? addr < sym.st_value + 0x1000 : addr < (&sym)[1].st_value) && (sym.st_size == 0 || addr <= sym.st_value + sym.st_size)) {
            if (!sym.st_value) {
                return false;
            }
            if (name) {
...
struct backtracer {
   ...
   private:
    ...
    void check() {
        if (rbp_ < rsp_ || stack_top_ - rbp_ < 16 || ((vmiter(pt_, rbp_).range_perm(16)) & PTE_P) == 0) {
            rbp_ = rsp_ = 0;
        }
    }
};
...
__noinline int error_vprintf(int cpos, int color, const char* format, va_list val) {
    va_list val2;
...
[[noreturn]] static void panic(uintptr_t rsp, uintptr_t rbp, uintptr_t rip,
                               const char* format, ...) {
    va_list val;
    va_start(val, format);
...
program_image::program_image(int program_number) {
    elf_ = nullptr;
    if (program_number >= 0 && size_t(program_number) < sizeof(ramimages) / sizeof(ramimages[0])) {
...
static elf_program* elf_header_program(elf_header* elf, bool end) {
    if (elf) {
        elf_program* ph = reinterpret_cast<elf_program*>(reinterpret_cast<uint8_t*>(elf) + elf->e_phoff);
