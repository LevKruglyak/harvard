...
#define NUMBUFSIZ 24
    ...
    for (; *format; ++format) {
        ...
        } else if ((flags & FLAG_NUMERIC) && (flags & FLAG_ZERO) && !(flags & FLAG_LEFTJUSTIFY) && datalen + (int)strlen(prefix) < width) {
...
__noinline void console_printer::scroll() {
...
__noinline void console_printer::move_cursor() {
...
__noinline int console_puts(int cpos, int color, const char* s, size_t len) {
...
__noinline int console_vprintf(int cpos, int color, const char* format, va_list val) {
    console_printer cp(cpos, cpos < 0);
...
__noinline int console_printf(int cpos, int color, const char* format, ...) {
    va_list val;
    va_start(val, format);
...
__noinline void console_printf(int color, const char* format, ...) {
    va_list val;
    va_start(val, format);
...
__noinline void console_printf(const char* format, ...) {
    va_list val;
    va_start(val, format);
...
__noinline int error_printf(int cpos, int color, const char* format, ...) {
    va_list val;
    va_start(val, format);
...
__noinline void error_printf(int color, const char* format, ...) {
    va_list val;
    va_start(val, format);
...
__noinline void error_printf(const char* format, ...) {
    va_list val;
    va_start(val, format);
