...
# Execute
    [ # Each test is defined as an array with components:
      ...
      # CMD_FILE: If set, store command in a file, not stdin.
...
# Command lists
    ...
    [ 'Test LIST6',
      ...
      CMD_CLEANUP => "sleep 0.2 ; ps t $TTY | grep -cm1 \"slee*p\""],
...
# Pipelines
    ...
    [ 'Test PIPE14',
      ...
      'Hello 1',
      CMD_INIT => 'echo "echo Hello; sleep 0.4" > cmd%%.sh',
      CMD_CLEANUP => "echo \"sleep 0.2 ; ps t $TTY | grep -cm1 \\\"slee*p\\\"\" > cmd%%b.sh; ../sh61 -q cmd%%b.sh",
    ...
    [ 'Test PIPE24', # actually a background test
...
sub run (@) {
    ...
    if (($result eq $want
        ...
        $in =~ s/\n/ \\n /g;
        print OUT "    Checking for $longdesc\n";
