...
void warp_thread(pong_warp* w) {
    ...
    while (true) {
        ...
        while (w->warp_balls.empty()) {
            w->warp_cv.wait(w->warp_mutex);
        }

        pong_ball* b = w->warp_balls.front();
        w->warp_balls.pop_front();
        ...
        w->board.board_mutex[b->y+1].lock();
        w->board.cell(b->x, b->y).ball = b;
        w->board.board_mutex[b->y+1].unlock();
...
int compute_direction(pong_board* b) {
    int delta = 0;
    int balls = 0;

    for (int x = 0; x < b->width; ++x) {
        for (int y = 0; y < b->height; ++y) {
            b->board_mutex[y+1].lock();
            if (b->cell(x, y).ball) {
                delta += x;
                ++balls;
            }
            b->board_mutex[y+1].unlock();
        }
    }
...
void paddle_thread(pong_board* b, int px, int py, int pw) {
    ...
    while (true) {
        ...
        dx = compute_direction(b) - (px + pw/2);
